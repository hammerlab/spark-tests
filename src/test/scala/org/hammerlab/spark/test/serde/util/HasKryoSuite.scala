...
import org.hammerlab.spark.test.serde.KryoSerialization.{ kryoBytes, kryoRead }
...
trait HasKryoSuite
  ...
  def checkKryoRoundTrip[T <: AnyRef : ClassTag](t: T,
                                                 size: Int,
                                                 includeClass: Boolean = false): Unit = {
  ...
  def checkKryoRoundTrip[T <: AnyRef : ClassTag](size: Int,
                                                 ts: T*): Unit =
